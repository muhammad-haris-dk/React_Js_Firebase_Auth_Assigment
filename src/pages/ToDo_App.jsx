...
function ToDo_App() {
  ...
  const bg_color =
    state_value.theme === "dark" ? "transparent-black" : "transparent";
  const color = state_value.theme === "dark" ? "text-white" : "text-black";
  const border =
    state_value.theme === "dark" ? "border-white" : "border-slate-500";
  ...
  return (
    <div>
      <div className="flex justify-center mt-20">
        <div
          className={` border duration-500 border-slate-500 p-5 rounded-lg h-[500px] ${bg_color}`}
        >
          <div className="flex">
            <div>
              <h1 className={`text-2xl ${color} duration-500 mb-5`}>
                This is my Persnol ToDo App?
              </h1>
            </div>
            <div className="mt-1 mx-2">
              <button
                className={`border ${background} ${colory} ${border} px-3 rounded-md`}
                onClick={theme_toggle_handle_dynamic}
              >
                {state_value.theme}
              </button>
            </div>
          ...
          <div className="">
            ...
            <div
              className={`border ${border} duration-500 mt-5 border-e-0 border-s-0 border-t-0`}
            ></div>
            <div
              className={`border rounded-md duration-500 ${border} mt-5 h-[300px] ok overflow-auto`}
            >
              {todo_List.length == 0 ? (
                <div className="flex justify-center items-center h-full">
                  <h1 className={`text-3xl ${color}`}>
                    Entired Feild Is Empty
                  </h1>
                </div>
              ) : (
                    ...
                    >
                      {data ? (
                        <div className="flex justify-between">
                          <div>
                            <p
                              className={`duration-500 ${border} py-5  text-lg  ${color}`}
                            >
                              {todo.task}
                            </p>
                          </div>
